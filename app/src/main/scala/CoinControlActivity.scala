...
class CoinControlActivity extends BaseCheckActivity with ExternalDataChecker {
  ...
  val chanAdapter: BaseAdapter = new BaseAdapter {
    ...
    def getView(position: Int, savedView: View, parent: ViewGroup): View = {
      val view =
        if (null == savedView)
          getLayoutInflater.inflate(R.layout.frag_utxo_line, null)
        else savedView.asInstanceOf[View]
      val holder =
        if (null == view.getTag) new UtxoHolder(view)
        else view.getTag.asInstanceOf[UtxoHolder]
  ...
  class UtxoHolder(itemView: View) extends RecyclerView.ViewHolder(itemView) {
    val txLabelOrId: TextView =
      itemView.findViewById(R.id.txLabelOrId).asInstanceOf[TextView]
    val utxoWrap: RelativeLayout =
      itemView.findViewById(R.id.utxoWrap).asInstanceOf[RelativeLayout]
    val utxoCardContainer: CardView =
      itemView.findViewById(R.id.utxoCardContainer).asInstanceOf[CardView]

    val utxoIncluded: CheckBox =
      itemView.findViewById(R.id.utxoIncluded).asInstanceOf[CheckBox]
    val utxoHaikuName: TextView =
      itemView.findViewById(R.id.utxoHaikuName).asInstanceOf[TextView]
    val utxoAmount: TextView =
      itemView.findViewById(R.id.utxoAmount).asInstanceOf[TextView]
    ...
    def setUtxoView(item: UnspentOutputLine): Unit = {
      ...
      val humanAmount =
        HubActivity.incoming(item.utxo.item.value.sat.toMilliSatoshi)
  ...
  def updateItems(unExcludedUtxos: Seq[Utxo] = Nil): Unit =
    items = unExcludedUtxos
      ...
      .flatMap { case (txid, unspents) =>
        val outPointsLine =
          for (unspentOutput <- unspents) yield UnspentOutputLine(unspentOutput)
  ...
  override def checkExternalData(whenNone: Runnable): Unit =
    InputParser.checkAndMaybeErase {
      case wallet: ElectrumEclairWallet
          if LNParams.chainWallets
