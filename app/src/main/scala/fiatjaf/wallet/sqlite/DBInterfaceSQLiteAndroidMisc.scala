package com.fiatjaf.wallet.sqlite

import immortan.sqlite._
import android.database.sqlite._
import android.content.Context

class DBInterfaceSQLiteAndroidMisc(context: Context, name: String)
    extends SQLiteOpenHelper(context, name, null, 3)
    with DBInterfaceSQLiteAndroid {
  ...
  def onUpgrade(dbs: SQLiteDatabase, v0: Int, v1: Int): Unit = {
    LNUrlPayTable.createStatements.foreach(
      dbs.execSQL
    ) // 1 -> 2 migration creates LNURL-PAY table
    LogTable.createStatements.foreach(
      dbs.execSQL
    ) // 1 | 2 -> 3 migration creates error log table
  }
}
