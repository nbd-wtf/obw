...
import com.fiatjaf.wallet.ClassNames
...
object DelayedNotification {
  ...
  def schedule(
    ...
    val constraintBuilder = (new Constraints.Builder)
      .setTriggerContentMaxDelay(1, TimeUnit.MILLISECONDS)
      .build
    val dataBuilder =
      (new Data.Builder).putString("title", title).putString("body", body)
  ...
  def cancel(context: Context, tag: String): Unit =
    WorkManager.getInstance(context).cancelAllWorkByTag(tag)

  class NotificationSchedule(context: Context, params: WorkerParameters)
      extends Worker(context, params) {
    private def setNotification(
        context: Context,
      ...
      val notificationBuilder =
        new NotificationCompat.Builder(context, CHANNEL_ID)
          .setSmallIcon(com.fiatjaf.wallet.R.drawable.baseline_feedback_24)
